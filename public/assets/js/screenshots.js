...
tabs.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    tabs.forEach((t) => {
      ...
      t.className = active
        ? "screenshot-tab rounded-full border border-brand-700 bg-brand-700 px-4 py-1.5 text-sm font-medium text-white transition-colors"
...
(function () {
  ...
  const dotClass = {
    active: "h-2 w-6 rounded-full bg-brand-500 transition-all duration-300",
